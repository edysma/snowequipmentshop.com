...
app()->booted(function () {
    ...
    if (is_plugin_active('ads')) {
        ...
        function display_ads_advanced(?string $key, array $attributes = []): ?string
        {
            ...
            $allowed_countries = explode(",",$ads->country_ids);
            $countryName = '';
            ...
            if(in_array($countryName,$allowed_countries)){
                ...
                if ($ads->url) {  
                    ...
                    $image = Html::link(route('public.ads-click', $ads->key), $image, array_merge($attributes, ['target' => $trg]), null, false)
                        ->toHtml();
                } elseif ($attributes) {
