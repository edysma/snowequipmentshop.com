<div class="ms-md-5 ps-md-5 form">
    ...
    <form class="mt-5 contact-form" action="{{ route('public.send.contact') }}" method="POST" role="form">
        ...
        <?php
            ...
            // print_r($url);
        ?>
        <input type="hidden" name="lang" value="<?=  $url[1]!='products' && $url[1]!='contact'?$url[1]:'it' ?>" />
        <input type="hidden" name="country" value="<?= get_application_country()->name; ?>" />
        <?php 
          $protocol =  "https://";  
