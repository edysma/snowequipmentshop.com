...
class DiscountController extends BaseController
{
    ...
    public function store(DiscountRequest $request, BaseHttpResponse $response)
    {
        ...
        if ($discount) {
            ...
            if($request->has('customers_category')){
                $customers_category=$request->input('customers_category');
            
                if(count($customers_category)>=1){
                   
                    $discount->customersCategory()->attach(array_unique($customers_category));
                    //$con=mysqli_connect("127.0.0.1","snow","Snow@2022!","snow",3306);
                    // mysqli_query($con,"INSERT INTO `ec_discount_customer_category`(`discount_id`, `customer_category_id`) VALUES()");
                }
            }
