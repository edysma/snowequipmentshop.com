...
class OrderController extends BaseController
{
        ...
        if ($order) {
            ...
            if ($request->input('customer_address.name')) {
                $this->orderAddressRepository->create([
                    ...
                    'partita_iva'     => $request->input('customer_address.partita_iva'),
                    'codice_fiscale'     => $request->input('customer_address.codice_fiscale'),
                    'codice_sdi'     => $request->input('customer_address.codice_sdi'),
                ...
                if ($request->input('customer_address.partita_iva') !== null){
                    $partita_iva = $request->input('customer_address.partita_iva');
                    $codice_fiscale = $request->input('customer_address.codice_fiscale');
                    $codice_sdi = $request->input('customer_address.codice_sdi');                
                }
    ...
    public function postSendOrderRecoverEmail($id, BaseHttpResponse $response)
    {
        ...
        try {
            ...
            $mailer->sendUsingTemplate($lng.'_order_recover', $email);
