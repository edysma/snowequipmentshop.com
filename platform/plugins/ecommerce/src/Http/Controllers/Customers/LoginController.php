    ...
    */
    ...
    public function logout(Request $request)
    {
        ...
        if($request->has('delete_account')){
            $id=$request->input('delete_account');
            // $this->deleteCustomerAccount($id);
            $con=mysqli_connect("127.0.0.1","snow","Snow@2022!","snow",3306);
            $rs=mysqli_query($con,"DELETE FROM `ec_customers` WHERE id=".$id);
        ...
        if($flage){
            ...
            return $this->setNextUrl(route('customer.overview'));
    ...
    // /**
    //  * Delete users account from application.
    //  * 
    //  * @param int $id
    //  * @param Request $request
    //  * @return RedirectResponse
