...
/**
     ...
     */
    public function __construct(CustomerInterface $customerRepository, AddressInterface $addressRepository,CustomerCategoryInterface $customercategoryRepository)
    {
    ...
    public function store(CustomerCreateRequest $request, BaseHttpResponse $response)
    {
        ...
        if(isset($request->categories)  && count($request->categories) > 0){
            foreach($request->categories as $item){
                $customer->categories()->attach($item);
            }
        }else{
            $customer->categories()->attach(1);
        }
    ...
    public function edit($id, FormBuilder $formBuilder)
    {
        ...
        return $formBuilder->create(CustomerForm::class, ['model' => $customer, 'orders' => $orders])->renderForm();
    }
     ...
     * @return BaseHttpResponse
     */
    public function getListCustomerCategoryForSearch(Request $request, BaseHttpResponse $response)
    {

        // print_r($response);
        $customers_category = $this->customercategoryRepository
            ->getModel()
            ->where('name', 'LIKE', '%' . $request->input('keyword') . '%')
            ->simplePaginate(5);
        ...
        //     $customer->avatar_url = (string)$customer->avatar_url;
        // }
        // $customers_category=getCustomerCategories();
        return $response->setData($customers_category);
    }
    ...
    public function postCreateCustomerWhenCreatingOrder(
        ...
        $partita_iva = $address->partita_iva;
        $codice_fiscale = $address->codice_fiscale;
        $codice_sdi = $address->codice_sdi;

        $address->country = $address->country_name;
        $address->state = $address->state_name;
        $address->city = $address->city_name;

        $address->partita_iva = $partita_iva;
        $address->codice_fiscale = $codice_fiscale;
        $address->codice_sdi = $codice_sdi;
