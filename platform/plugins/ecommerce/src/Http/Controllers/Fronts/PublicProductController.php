...
class PublicProductController
{
    ...
    public function getProductsxml(Request $request, GetProductService $productService, BaseHttpResponse $response)
    {
        ...
        $countrycode = DB::table('countries')->where('code',$_GET['c'])->first();
        $shipping = DB::table('ec_shipping_rules')->where('shipping_id',$countrycode->id)->first();
        $currency = DB::table('ec_currencies')->where('title',$_GET['currency'])->first();
       
        if(empty($currency)){
            $currency = DB::table('ec_currencies')->where('is_default','1')->first();
        }

        $lang=DB::table('languages')->where('lang_locale',$_GET['lang'])->first();
        if(empty($lang)){
            $lang = DB::table('languages')->where('lang_is_default','1')->first();
        }
        ...
        echo '<channel>
            <title>Snow Equipment Shop</title>
            <link>https://snowequipmentshop.com</link>
            <description>Macchinari e accessori per il ripristino della viabilità stradale e la riqualificazione urbana.</description>';
        foreach($products as $product){
            $priceInfo = DB::table('ec_product_price')->where('ec_products_id',$product->id)->where('countries_id',$countrycode->id)->first();
            $brand = DB::table('ec_brands')->where('id',$product->brand_id)->first();
            $brandname = "";
            ...
            $_product = DB::table('ec_products_translations')->where('lang_code',$lang->lang_code)->where('ec_products_id',$product->id)->first();
            if(empty($_product)){
                $_product=$product;
            }

            echo '<item>';
            echo '<g:id>'.$product->sku.'</g:id>';
            echo '<g:title>'.$_product->name.'</g:title>';
            echo '<g:description>'.htmlspecialchars($_product->description).'</g:description>';
            
			///inizio modifica Marco Gallerani Edysma 12/01/2023  per assenza codice lingua in url
			$lingua2 = substr($lang->lang_code, 0, 2).'/';
			if ($lingua2 == "it/"){ $lingua2 = "";}
			$linklang = substr_replace($product->url, $lingua2, 43, 0);
			echo '<g:link>'.$linklang.'</g:link>';
			///fine modifica Marco Gallerani Edysma 12/01/2023
			
			//echo '<g:link>'.$product->url.'</g:link>';
			echo '<g:image_link>https://snowequipmentshop.com/storage/'.$product->image.'</g:image_link><g:condition>new</g:condition>';
            echo '<g:availability>'.$product->stock_status.'</g:availability>';
            if(isset($priceInfo->price)){
                echo '<g:price>'.format_pricexml($priceInfo->price,$currency->id).'</g:price>';
            }
			//  remmate su richiesta ADV EXPERT il 13/01/20223
			
           	// echo '<g:shipping>';
			//echo '<g:max_transit_time>5gg</g:max_transit_time>';
            //echo '<g:max_handling_time>2gg</g:max_handling_time>';
            //echo '<g:country>'.$countrycode->name.'</g:country>';
			
            //già remmato//echo '<g:service>'.$shipping->name.'</g:service>';
            //echo '<g:price>'.format_pricexml($shipping->price,$currency->id).'</g:price>';

            //echo '</g:shipping>';
            echo '<g:gtin>'.$product->ean.'</g:gtin>';
              echo '<g:brand>'.$brandname.'</g:brand>';
            echo '</item>';
        }
        echo "</channel></rss>";
        exit;
    ...
    public function getProductVariation($id, Request $request, BaseHttpResponse $response)
    {
        ...
        $price = $product->original_product->price;
        $country_id = 1;
        if(session('country') != '') {
            $country_id = session('country');
        }

        $priceInfo = DB::table('ec_product_price')->where('ec_products_id',$product->original_product->id)->where('countries_id',$country_id)->first();
        
        if($priceInfo) {
            $price = $priceInfo->price;
        }
        $product->original_product->price = $price;

        $data = (new ProductVariationResource($product));
