    ...
    public function getView(HandleApplyPromotionsService $applyPromotionsService)
    {
        if (isset($_GET['country'])){
            foreach (\Botble\Location\Models\Country::all() as $country){
                ...
                if(strtolower($country->code) == strtolower($_GET['country'])) {
                    // dd('running');
        ...
        if (Cart::instance('cart')->count() > 0) {
            ...
            $crossSellProducts = get_cart_cross_sale_products($parentIds, theme_option('number_of_cross_sale_product', 200));
        }
