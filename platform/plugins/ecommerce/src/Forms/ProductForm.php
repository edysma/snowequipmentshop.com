...
class ProductForm extends FormAbstract
{
    ...
    public function buildForm()
    {
        Assets::addStyles(['datetimepicker'])
            ...
            ->addScriptsDirectly([
                'vendor/core/plugins/ecommerce/js/edit-product.js?v=1.6.0',
            ]);
        ...
        if ($this->getModel()) {
            ...
            $productPrice = ProductPrice::where('ec_products_id',$productId)->get();
        ...
        $this
            ...
            ->add('ean', 'text', [
                'label'      => trans('plugins/ecommerce::products.form.ean'),
                'label_attr' => ['class' => 'text-title-field'],
                'attr'       => [
                    'placeholder'  => trans('plugins/ecommerce::products.form.ean'),
                    'data-counter' => 150,
                ],
            ])
        ...
        if (empty($productPrice) || $productPrice->isEmpty()) {
            $attributeSetId = $productAttributeSets->first() ? $productAttributeSets->first()->id : 0;

            $this->addMetaBoxes([
                'prices' => [
                    'title'         => trans('plugins/ecommerce::products.prices'),
                    'content'       => view('plugins/ecommerce::products.partials.add-product-prices', [
                        'productAttributeSets' => $productAttributeSets,
                        'productAttributes'    => $this->getProductAttributes($attributeSetId),
                        'attributeSetId'       => $attributeSetId,
                        'country'              => Country::all(),
                    ...
                    'before_wrapper' => '<div id="main-manage-product-price">',
                    'after_wrapper' => '</div>',
                    'priority'      => 4,
                ],
            ]);
            ...
            $this->removeMetaBox('prices')
            ->addMetaBoxes([
                'variations1' => [
                    'title'          => trans('plugins/ecommerce::products.product_has_nation_prices'),
                    'content'        => view('plugins/ecommerce::products.partials.nation-prices', [
                        'productAttributeSets'       => $productAttributeSets,
                        'productVariations'          => $productVariations,
                        'productVariationsInfo'      => $productVariationsInfo,
                        'productsRelatedToVariation' => $productsRelatedToVariation,
