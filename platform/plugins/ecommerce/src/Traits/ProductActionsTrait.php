...
trait ProductActionsTrait
{
    ...
    public function postAddVersion(
        ...
        if($request->type == 'price'){
            ...
            $price->ec_products_id = $id;
            $price->countries_id = $request->country_id;
    ...
    public function getVersionForm(
        ...
        $originalProduct = $this->productRepository->findOrFail($request->ref_from);
        ...
        if($request->type == 'price' ){
            if($id != 0){
                ...
                $productId = $variation ? $variation->configurable_product_id :$request->ref_from;
                ...
                return $response
                    ->setData(
                        view('plugins/ecommerce::products.partials.product-price-edit-form', compact(
                ...
                return $response
                    ->setData(
                        view('plugins/ecommerce::products.partials.product-price-add-form', compact(
                            'originalProduct',
                            'countries',
                            'id'
                        ))->render()
    ...
    public function postUpdateVersion(
        ...
        if ($request->type == 'price') {
            ...
            $price->countries_id = $request->country_id;
            $price->price = $request->price;
            $price->nation_sale_price = $request->nation_sale_price;
            $price->save();
            return $response->setMessage(trans('plugins/ecommerce::products.form.updated_price_success'));
