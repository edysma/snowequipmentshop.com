            ...
            ->where(function ($query) use ($forProductSingle) {
                ...
                return $query
                    ...
                    ->orWhere(function ($sub) use ($forProductSingle) {
                        ...
                        return $sub
                            ->whereIn('target', ['customer', 'group-products', 'specific-product', 'product-variant','customer-category'])
            ...
            ->where(function ($query) use ($productIds, $productCollectionIds) {
                ...
                return $query
                    ...
                    ->orWhere(function ($sub) {
                         ...
                         */
                        return $sub
                            ->where('target', 'customer-category');
                            // ->whereHas('customers', function ($whereHas) {
                            //     $customerId = auth('customer')->check() ? auth('customer')->id() : -1;

                            //     return $whereHas->where('ec_discount_customers_cateogry.customer_category_id', 1);
                            // });
                    });
