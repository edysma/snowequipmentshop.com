...
if (!function_exists('format_pricexml')) {
    /**
     ...
     * @return string
     */
    function format_pricexml($price, $currency = null, bool $withoutCurrency = false, bool $useSymbol = true): string
    {
        ...
        if ($currency) {
            ...
            //   }else{
            //     $price = $price / $currency->exchange_rate * $currency->exchange_rate;
            //   }
        ...
        if ($useSymbol && $currency->is_default) {
            $space = get_ecommerce_setting('add_space_between_price_and_currency', 0) == 1 ? ' ' : null;
...
if (!function_exists('get_application_country')) {
    /**
     * @return Currency|null
     */
    function get_application_country(): ?Country
    {
        $country = cms_currency()->getApplicationCountry();

        if (is_in_admin() || !$country) {
            $country = Country::all();
        }
...
if (!function_exists('get_application_country_id')) {
    /**
     * @return int|null
     */
    function get_application_country_id(): ?int
    {
        return get_application_country()->id;
    }
}
