<div class="form-group mb-3">
    <input type="hidden" name="discount_products" value="@if ($flashSale->id){{ implode(',', array_filter($flashSale->products()->allRelatedIds()->toArray())) }}@endif" />
    <div class="row">
        <div class="col-md-6">
            <label class="control-label required" aria-required="true">Categorie</label>


            <select class="form-control select-full ui-select select2-hidden-accessible" name="customer_categorie[]" id="">
                <option value="sd">df</option>
            </select>
        ...
        <div class="col-md-6">
            <div class="form-group mb-3">
                <label class="control-label required" aria-required="true">Discount</label>
                <input type="text" class="next-input " placeholder="discount" required>
            </div>
        </div>
    </div>
                    ...
                    <tr data-product-id="{{ $product->id }}">
                        <td class="width-60-px min-width-60-px" style="padding-top: 15px;">
                            <div class="wrap-img vertical-align-m-i">
                                <img class="thumb-image" src="{{ RvMedia::getImageUrl($product->image, 'thumb', false, RvMedia::getDefaultImage()) }}" alt="{{ $product->name }}">
                            </div>
                        </td>
        ...
        <td colspan="3" style="border-top: none; border-bottom: 1px solid #ececec;">
            <div class="row">
                ...
                <div class="col-6">
                    <div class="form-group mb-3">
                        <label class="control-label required">{{ trans('plugins/ecommerce::products.quantity') }}</label>
                        <input type="text" class="form-control" name="discount_products_extra[__index__][quantity]" value="1">
                    </div>
                </div>
            </div>
        </td>
