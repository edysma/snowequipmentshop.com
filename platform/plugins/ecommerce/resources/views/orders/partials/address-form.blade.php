<div class="customer-address-payment-form">
    ...
    <div class="row">
        ...
        <div class="row">
            <div class="col-12">
                <div class="form-group mb-3 @if ($errors->has('address.partita_iva')) has-error @endif">
                    <input type="text" name="address[partita_iva]" id="address_partita_iva" placeholder="{{ __('Partita IVA') }}" class="form-control address-control-item address-control-item-required checkout-input"
                           value="{{ old('address.partita_iva', Arr::get($sessionCheckoutData, 'partita_iva')) }}">
                    {!! Form::error('address.partita_iva', $errors) !!}
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-lg-6 col-12">
                <div class="form-group  @if ($errors->has('address.codice_fiscale')) has-error @endif">
                    <input type="text" name="address[codice_fiscale]" id="address_codice_fiscale" placeholder="{{ __('Codice Fiscale') }}" class="form-control address-control-item address-control-item-required checkout-input" value="{{ old('address.codice_fiscale', Arr::get($sessionCheckoutData, 'codice_fiscale')) }}">
                    {!! Form::error('address.codice_fiscale', $errors) !!}
                </div>
            </div>
            <div class="col-lg-6 col-12">
                <div class="form-group  @if ($errors->has('address.codice_sdi')) has-error @endif">
                    <input type="text" name="address[codice_sdi]" id="address_codice_sdi" placeholder="{{ __('Codice SDI') }}" class="form-control address-control-item address-control-item-required checkout-input" value="{{ old('address.codice_sdi', Arr::get($sessionCheckoutData, 'codice_sdi')) }}">
                    {!! Form::error('address.codice_sdi', $errors) !!}
                </div>
            </div>
        </div>
            ...
            @if (EcommerceHelper::isUsingInMultipleCountries())
                ...
                <div class="col-12">
                    <div class="form-group mb-3 @if ($errors->has('address.country')) has-error @endif">
                        <div class="select--arrow">
                            <select name="address[country]" class="form-control address-control-item address-control-item-required" id="address_country" data-type="country" onchange="changeCountryTrigger()" >
                                @foreach(EcommerceHelper::getAvailableCountries() as $countryCode => $countryName)
                                    <option value="{{ $countryCode }}" @if (session()->all()['countryCode'] == $countryCode) selected @endif>{{ $countryName }}</option>
                                @endforeach
                            </select>
                            <i class="fas fa-angle-down"></i>
                        </div>
...
@push('footer')
    <script>
          var activities = document.getElementById("address_country");
            activities.addEventListener("change", function() {
                window.location.href = "https://snowequipmentshop.com/en/cart?country=" + $('.form-checkout #address_country').val();
            });
            activities.addEventListener("change", changeCountryTrigger);
            function changeCountryTrigger() {
                window.location.href = "https://snowequipmentshop.com/en/cart?country=" + $('.form-checkout #address_country').val();
            }
