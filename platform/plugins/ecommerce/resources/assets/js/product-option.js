...
$(document).ready(function () {
    ...
    let productOptionForm = {
        ...
        eventListeners() {
            ...
            $('.product-option-form-wrap').on('click', '.remove-option', function () {
                const index = $(this).data("index");
                self.productOptions.splice(index, 1);
                $(this).parents(".accordion-item").remove();
            })

            $('.product-option-form-wrap').on('keyup', '.option-name', function () {
            ...
            $('.product-option-form-wrap').on('change', '.option-type', function () {
                const index = $(this).parents(".accordion-item").data('product-option-index');
                self.productOptions[index].option_type = $(this).val();
                self.generateProductOption();
            })

            $('.product-option-form-wrap').on('change', '.option-required', function () {
                const index = $(this).parents(".accordion-item").data('product-option-index');
                self.productOptions[index].required = $(this).is(":checked")
            })

            $('.product-option-form-wrap').on('click', '.add-new-row', function () {
                self.addNewRow($(this));
            })

            $('.product-option-form-wrap').on('click', '.remove-row', function () {
                $(this).parent().parent().remove();
            })

            $('.product-option-form-wrap').on('click', '.add-new-option', function () {
